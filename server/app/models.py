...
class AgentResponse(BaseModel):
    ...
    answer: str = Field(
        ..., min_length=1, description="Generated response from the LLM agent"
    )
...
class HistoryRequest(BaseModel):
    ...
    checkpoint_id: str | None = Field(None, description="Optional checkpoint ID")


class HistoryResponse(BaseModel):
    """Response model for conversation history."""

    messages: list[dict] = Field(..., description="List of historical messages")
