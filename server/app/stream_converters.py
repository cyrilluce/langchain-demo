...
class BaseConverter:
    ...
    def _emit_available(
        self, chunk_type: str, chunk_id: str, content: Any, **kwargs
    ) -> str:
...
class ModelConverter(BaseConverter):
    ...
    async def convert(self, chunk: AIMessageChunk) -> AsyncIterator[str]:
        ...
        reasoningList = [
            block for block in chunk.content_blocks if block.get("type") == "reasoning"
        ]
    ...
    async def _handle_text(self, content: str | dict | list) -> AsyncIterator[str]:
    ...
    async def _handle_tool_call_chunk(self, tool_chunk: dict) -> AsyncIterator[str]:
        """
        ...
        if index not in self.tool_call_ids:
            ...
            yield self._emit_start(
                "tool-input", chunk_id, toolCallId=chunk_id, toolName=name
            )
    ...
    async def _finalize_all(self) -> AsyncIterator[str]:
        ...
        for index, chunk_id in self.tool_call_ids.items():
            ...
            try:
                ...
                parsed_input = (
                    json.loads(accumulated_input) if accumulated_input else {}
                )
...
class ToolsConverter(BaseConverter):
    ...
    async def finalize_tool(self, tool_call_id: str) -> AsyncIterator[str]:
        ...
        if tool_call_id in self.tool_output_ids:
            self.tool_output_ids[tool_call_id]
...
class StreamConverter:
    ...
    async def convert_stream(
        ...
        if not self.message_id:
            ...
            yield self._format_sse({"type": "start", "messageId": self.message_id})
