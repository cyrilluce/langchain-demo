...
class TestModelConverter:
    ...
    @pytest.mark.asyncio
    async def test_text_conversion(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_reasoning_conversion(self) -> None:
        """Test reasoning chunk conversion."""
        converter = ModelConverter()

        # Create chunk with reasoning attribute
        chunk = AIMessageChunk(
            content=[dict({"type": "reasoning", "reasoning": "Let me think..."})]
        )
        ...
        parsed_events = [
            json.loads(e.replace("data: ", "").strip()) for e in events if e.strip()
        ]
    ...
    @pytest.mark.asyncio
    async def test_tool_call_chunk_conversion(self) -> None:
        ...
        chunk2 = AIMessageChunk(
            content="",
            tool_call_chunks=[{"index": 0, "name": "", "args": '{"city":', "id": ""}],
        )
        chunk3 = AIMessageChunk(
            content="",
            tool_call_chunks=[{"index": 0, "name": "", "args": ' "SF"}', "id": ""}],
        )
...
class TestToolsConverter:
    ...
    @pytest.mark.asyncio
    async def test_tool_output_conversion(self) -> None:
        """Test tool output message conversion."""
        converter = ToolsConverter()

        # Create tool message
        tool_msg = ToolMessage(content="The weather is sunny", tool_call_id="call_123")
    ...
    @pytest.mark.asyncio
    async def test_tool_output_json_content(self) -> None:
        ...
        tool_msg = ToolMessage(content=json_content, tool_call_id="call_456")
...
class TestStreamConverter:
    ...
    @pytest.mark.asyncio
    async def test_full_stream_conversion(self) -> None:
    ...
    async def test_stream_with_reasoning(self) -> None:
        ...
        async def mock_stream():
            ...
            chunk = AIMessageChunk(
                content=[{"type": "reasoning", "reasoning": "I think..."}, "Answer"]
            )
            yield chunk
...
class TestUIMessageStreamHelpers:
    ...
    def test_extract_prompt_from_messages(self) -> None:
        ...
        messages = [{"id": "msg2", "role": "user", "content": "Fallback content"}]
    ...
    def test_extract_prompt_error_cases(self) -> None:
